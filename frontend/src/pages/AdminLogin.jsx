      ...
      <div className="mx-auto flex min-h-[85vh] w-full max-w-md items-center justify-center">
        <div className="w-full rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-xl">
          <div className="mb-8 text-center">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-lime-400 text-xl font-black text-black">CGF</div>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-lime-400">Administrator</p>
            <h1 className="mt-2 text-3xl font-black">Admin Login</h1>
            <p className="mt-2 text-sm text-gray-500">Sign in to access the CGF admin dashboard.</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label htmlFor="admin-email" className="mb-2 block text-sm font-medium text-gray-300">Administrator Email</label>
              <input
                id="admin-email"
                type="email"
                value={email}
            ...
            <div className="text-right">
              <Link to="/admin-forgot-password" className="text-sm font-semibold text-gray-300 underline underline-offset-4 hover:text-white">
                Forgot Password?
              </Link>
            </div>
            ...
            <button type="submit" disabled={loading} className="w-full rounded-xl bg-lime-400 px-4 py-3 font-black text-black transition hover:bg-lime-300 disabled:cursor-not-allowed disabled:opacity-60">
              {loading ? "Signing in..." : "ADMIN SIGN IN"}
            </button>
          </form>

          <button type="button" onClick={() => navigate("/login")} className="mt-6 w-full text-center text-xs font-bold text-gray-500 transition hover:text-white">
